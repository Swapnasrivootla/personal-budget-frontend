  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const endpoint = `http://localhost:4200/get-budgets/${userData}?month=${selectedMonth}`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const token = localStorage.getItem('token');
        const userData = localStorage.getItem('userData');
        const endpoint = `http://localhost:4200/get-expenses/${userData}?month=${selectedMonth}`;
        const res = await axios.get(endpoint, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
